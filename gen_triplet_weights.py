...
with open('/home/knappa/Downloads/alignment_GTR_30_taxa_1K_sites_rtree1.phy', 'r') as file:
...
with open('test.txt','w') as file:
    for a_species, b_species, c_species in itertools.combinations(data.keys(), 3):
        ...
        for k in range(4):
            ...
            svd_score_ab += np.sum(np.linalg.svd(M_ab[k, :, :], compute_uv=False)[9:] ** 2)
        ...
        for k in range(4):
            ...
            svd_score_ac += np.sum(np.linalg.svd(M_ac[k, :, :], compute_uv=False)[9:] ** 2)
        ...
        for k in range(4):
            ...
            svd_score_bc += np.sum(np.linalg.svd(M_bc[k, :, :], compute_uv=False)[9:] ** 2)
